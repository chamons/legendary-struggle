...
namespace LS.Core
{
	public static class EnumerableExtensions
	{
		...
		public static T WithID<T> (this IEnumerable<T> array, T item) where T : IIdentifiable
		{
			return array.First (x => x.ID == item.ID);
		}

		public static ImmutableArray<T> ReplaceWithID<T> (this ImmutableArray<T> array, T item) where T : IIdentifiable
		{
			return array.Replace (array.WithID (item), item);
		}
	}
}
